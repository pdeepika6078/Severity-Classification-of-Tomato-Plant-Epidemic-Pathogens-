...
class FPN(nn.Module):
    def __init__(self, C1, C2, C3, C4, C5, out_channels):
        ...
        self.P5_conv1 = nn.Conv2d(int(sol[0]), self.out_channels, kernel_size=1, stride=1)
...
def detection_target_layer(proposals, gt_class_ids, gt_boxes, gt_masks, config):
    ...
    if torch.nonzero(positive_roi_bool).size():
        ...
        masks = Variable(
            CropAndResize(config.MASK_SHAPE[0], config.MASK_SHAPE[1], 0)(roi_masks.unsqueeze(1), boxes, box_ids).data,
            requires_grad=False)
...
class MaskRCNN(nn.Module):
    ...
    def __init__(self,sol, config, model_dir):
        """
    ...
    def train_model(self, train_dataset, val_dataset, learning_rate, epochs, layers, sol):
        """Train the model.
        ...
        train_generator = torch.utils.data.DataLoader(train_set, epochs = int(sol[1]),STEPS_PER_EPOCH= int(sol[2]), batch_size=1, shuffle=True, num_workers=4)
